...
public interface IStack<T> {
     ...
     */
    @Contract.Requires("value_not_null")
    @Contract.Ensures("size_increase")
    public boolean push(T value);

    default boolean value_not_null(T value){
        return value != null;
    }
    default boolean size_increase(){
        return this.size()-1 == old(size());
    }
     ...
     */
    @Contract.Ensures({"stack_is_same", "item_was_in_stack"})
    public T pop();

    @Contract.Pure
    default boolean stack_is_same(){
        if(size() > 0){
            return forAllInts(0, size(), idx -> pop() == old(this).pop());
        }
            return false;
    }

    @Contract.Pure
    default boolean item_was_in_stack(T returns){
        return returns == old(this).peek();
    }
    ...
    @Contract.Ensures("stack_is_empty")
    public void clear();
     ...
     */
    @Contract.Pure
    @Contract.Ensures("not_negative")
    public int size();

    @Contract.Pure
    default boolean not_negative(int returns){
        return returns > 0;
    }
