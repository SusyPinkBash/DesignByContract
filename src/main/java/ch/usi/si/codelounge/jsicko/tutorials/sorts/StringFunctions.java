...
public class StringFunctions {
    ...
    @Contract.Pure
    boolean shift_right_correct(int start, int end, char[] array){
    ...
    @Contract.Pure
    boolean end_more_than_start(int start, int end){
        return end > start;
    }
    ...
    @Contract.Pure
    boolean shift_left_correct(int start, int end, char[] array){
        for(int i = start; i< end; i++){
            if (array[i] == old(array)[i+1])
                continue;
            else
